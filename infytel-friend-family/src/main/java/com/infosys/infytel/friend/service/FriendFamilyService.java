...
public class FriendFamilyService {
	...
	public List<Long> getFamilyList(Long phoneNo){
		List<Long> phoneNoList = new ArrayList<>();
		List<FriendFamily> friendFamilyList = friendRepo.findByPhoneNo(phoneNo);
		for (FriendFamily friendFamily : friendFamilyList) {
			phoneNoList.add(friendFamily.getFriendAndFamily());
		}
		return phoneNoList;
	}
}
